...
const mobileScoreTwoElement = document.getElementById("mobileScoreTwo");
const mobileScoreTwoWrapElement = document.getElementById("mobileScoreTwoWrap");
const mobileHighscoreElement = document.getElementById("mobileHighscore");
const mobileMissionTitleElement = document.getElementById("mobileMissionTitle");
const mobileMissionProgressElement = document.getElementById("mobileMissionProgress");
...
function resetGame() {
  ...
  if (mode === "solo") {
    ...
    setMissionTexts("Duell aktiv", "Missionen sind im Zwei-Spieler-Modus pausiert.");
    currentMission = null;
  }
  spawnFood();
  startRenderLoop();
...
function setMobileScene(nextScene) {
  mobileScene = nextScene === "game" ? "game" : "settings";

  if (!isMobileViewport()) {
    document.body.classList.remove("mobile-scene-settings", "mobile-scene-game");
    return;
  }

  document.body.classList.toggle("mobile-scene-settings", mobileScene === "settings");
  document.body.classList.toggle("mobile-scene-game", mobileScene === "game");
}

function updateResponsiveScene() {
  setMobileScene(mobileScene);
}
...
function updateScoreUI() {
  ...
  mobileScoreTwoElement.textContent = scoreTwo;
  mobileHighscoreElement.textContent = highscore;
  const showSecondScore = mode === "duel";
  scoreTwoCardElement.classList.toggle("is-hidden", !showSecondScore);
  mobileScoreTwoWrapElement.classList.toggle("is-hidden", !showSecondScore);
}

function setMissionTexts(title, progress) {
  missionTitleElement.textContent = title;
  missionProgressElement.textContent = progress;
  mobileMissionTitleElement.textContent = title;
  mobileMissionProgressElement.textContent = progress;
}
...
function maybeCompleteMission() {
  ...
  updateHighscore();
  updateScoreUI();
  setMissionTexts("Mission geschafft!", `+${currentMission.reward} Bonuspunkte erhalten`);
  playSound("gold");
...
function updateMissionUI() {
  if (!currentMission) {
    setMissionTexts("Noch keine Mission", "Starte eine Runde, um eine Mission zu bekommen.");
    return;
  }

  setMissionTexts(currentMission.title(currentMission), currentMission.progress(currentMission));
}
